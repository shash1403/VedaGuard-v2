...
# Project root (vedaguard/): prefer .env.testnet so TestNet deploy matches AlgoKit docs.
_env_root = Path(__file__).resolve().parents[1]
logger.info("Loading .env.testnet / .env from %s", _env_root)
load_dotenv(_env_root / ".env.testnet")
...
def import_deploy_if_exists(folder: Path) -> Callable[[], Any] | None:
